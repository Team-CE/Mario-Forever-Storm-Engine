tool
class_name ComponentView extends Control
var editor_plugin:EditorPlugin
var search_term = ""

var search_in_all = false
var component_path = "res://Objects/"
var custom_component_path = "res://Games/"
...
func add_item(path,component_file,category):
	...
	icon = return_item_sprite(scene_instance)
	if !icon:
...
func return_item_sprite(scene_instance):
	for node in scene_instance.get_children():
		if node is Sprite:
			return node.texture
		elif node is AnimatedSprite:
			return node.frames.get_frame(node.animation, 0)
...
func list_files_in_directory(path,category=""):
	...
	while true:
		...
		#print(file)
		#print(dir.current_is_dir(), dir.get_current_dir() + '/' + file)
		if dir.current_is_dir() and file != '' and file != '.' and file != '..' and category:
			list_files_in_directory(dir.get_current_dir() + '/' + file, category + '/' + file)
		if file == "":
			break
...
func update_categories():
	...
	list_files_in_directory(custom_component_path)
